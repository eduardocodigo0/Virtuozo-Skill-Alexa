...
class cadastraFornecedorIntentHandler(AbstractRequestHandler):
    #PESSOA JURIDICA
    def can_handle(self, handler_input):
        return ask_utils.is_intent_name("cadastraFornecedorIntent")(handler_input)
    ...
    def handle(self, handler_input):
        ...
        url = session_attr['big_boost_url'] + "/companies"
        ...
        if not dados:
            ...
            session_attr['estado'] = 105 # FORNECEDOR
            session_attr['dados'] = dados
            
            speak_output = "O fornecedor {0} foi encontrada, deseja confirmar o cadastro?".format(dados['nome'])
        ...
        return (
                ...
                .speak(speak_output)
                .set_card(StandardCard("Cadastro de fornecedor", card_text, session_attr['imagem_padrao']))
...
class cadastrarPessoaJuridicaIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        return (
                handler_input.response_builder
                .speak(speak_output)
                .set_card(StandardCard("Cadastro de pessoa", card_text, session_attr['imagem_padrao']))
                .ask("Posso ajudar em mais alguma coisa?")
                .response
                )
...
class cadastrarPessoaIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        if len(documento) == 11:
            ...
            url = session_attr['big_boost_url'] + "/peoplev2"
            resposta = requests.get(url, headers=header, params=body)
